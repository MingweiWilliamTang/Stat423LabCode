...
forward.Select = step(lm.0,scope=~WT2 + HT2 + WT9 + HT9 + LG9 + ST9 + 
                      ...
                      direction = "forward",scale =sigmaHat(lm.13)^2)
forward.Select$call
...
step(lm.13,direction = "backward",scale=sigmaHat(lm.13)^2)
extractAIC(lm.13,scale=sigmaHat(lm.13)^2)[2]
